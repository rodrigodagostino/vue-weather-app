<template>
  <transition name="slide-fade-up" appear>
    <form
      @submit.prevent="fetchSearchData"
      class="search-form"
      :style="formStyles"
    >
      <input
      ...
      >
        ...
        <ul
          v-else-if="!isFetching && suggestions && suggestions.length > 0"
        ...
        >
          ...
          >
            <button
              type="button"
              @click="
                selectLocation(`${suggestion.name}, ${suggestion.sys.country}`)
              "
            >
...
watch(
  selectedLocation,
  async ( currSelectedLocation, prevSelectedLocation ) => {
    if (
      !prevSelectedLocation ||
      currSelectedLocation !== prevSelectedLocation
    ) {
...
.search-input {
  ...
  outline: none;
  transition: background-color 0.32s ease, border-radius 0.32s ease,
